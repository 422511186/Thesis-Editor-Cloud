 ...
 */
@Data
public class ArchiveOption extends BaseEntity {
     ...
     * 通过反射获取有效参数列表
     *
     * @param archiveOption 文档排版配置参数对象
     * @return options 有效参数列表
     * @throws IllegalAccessException
     */
    public static ArrayList<String> getOptionList(ArchiveOption archiveOption) throws IllegalAccessException {
        ArrayList<String> options = new ArrayList<>();
        //对象判空
        if (archiveOption != null) {
            for (Field field : archiveOption.getClass().getDeclaredFields()) {
                //参数判空
                if (field.get(archiveOption) != null)
                    options.add(String.valueOf(field.get(archiveOption)));
            }
        }
        return options;
    }
    ...
    public static void main(String[] args) throws IllegalAccessException {
        ArchiveOption option = new ArchiveOption();
        option.setColumn("colum");
        option.setScape("setScape");
        option.setPaper_size("setPaper_size");
        ArrayList<String> list = ArchiveOption.getOptionList(option);
